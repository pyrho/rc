...
function M.check_trigger_char(line_to_cursor, trigger_character)
  ...
  for _, ch in ipairs(trigger_character) do
    local current_char = string.sub(line_to_cursor, #line_to_cursor - #ch + 1,
                                    #line_to_cursor)
    if current_char == ch then return true end
    if current_char == " " and #line_to_cursor > #ch + 1 then
      local pre_char = string.sub(line_to_cursor, #line_to_cursor - #ch,
                                  #line_to_cursor - 1)
      if pre_char == ch then return true end
    end
  end
  return false
end
...
function M.lspsaga_open_signature_help()
  ...
  if triggered then vim.lsp.buf.signature_help() end

end

vim.cmd "augroup Signature"
vim.cmd "autocmd! * <buffer>"
vim.cmd "autocmd InsertCharPre * lua require'pyrho.plugins.conf.lspsaga_signature_help'.lspsaga_open_signature_help()"
vim.cmd "augroup end"
