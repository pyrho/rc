return {
	...
	config = function()
		...
		local ViMode = {
			...
			provider = function(self)
				...
				-- return " %2(" .. self.mode_names[vim.fn.mode(1)] .. "%)"
				return require("pyrho.helpers").separators.evil .. " %2(" .. self.mode_names[vim.fn.mode(1)] .. "%)"
			end,
			hl = function(self)
    ...
    { -- A special winbar for terminals
      ...
      utils.surround({"", ""}, colors.red1,
			...
			{ -- Disable winbar for zen mode
				condition = function()
					return require("pyrho.helpers").is_zen()
				end,
				provider = "",
			},
		...
		local Obsession = {
			-- flexible = flexHighPriority,
			{
				...
				on_click = {
					callback = function()
						vim.api.nvim_command("SessionStart")
					end,
  ...
  augroup END
		...
		-- This makes sure that the diagnostics get updated live {{{
		local heirlineAuGrp = vim.api.nvim_create_augroup("MyHeirlineAuGrp", { clear = true })
