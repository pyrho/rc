...
local function load_state()
  local state_file = string.format('%s/.prettierd', loop.os_getenv('XDG_RUNTIME_DIR'))
...
local function start_server()
  local exec_path = string.format('%s/langservers/prettierd',
                                  vfn.stdpath('config'))
...
local function wait_for_server(timeout_ms)
  ...
  if not status then
    error(string.format('server didnt start after %dms, status is %s', timeout_ms, status))
  end
end
